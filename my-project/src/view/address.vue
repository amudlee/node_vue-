  ...
  <div class="container">
    <div class="checkout-addr">
      ...
      <div class="addr-list-wrap">
        <div class="addr-list">
          <ul>
            <li v-for="(item,index) in changemore" :key="index" :class="{'check':checkIndex==index}" @click="checkIndex=index;selectedId=item.addressId">
              <dl>
                <dt>{{item.userName}}</dt>
              ...
              <div class="addr-opration addr-del">
                <a href="javascript:;" class="addr-del-btn" @click="delAd(item.addressId)">
                  <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                </a>
              </div>
              <div class="addr-opration addr-set-default" v-if="!item.isDefault">
                <a href="javascript:;" class="addr-set-default-btn" @click="setDefaultAd(item.addressId)"><i>设置默认地址</i></a>
              </div>
        ...
        <div class="shipping-addr-more">
          <a class="addr-more-btn up-down-btn" href="javascript:;" @click="getmore" :class="{'open':limit>3}">
            more
            <i class="i-up-down" >
      ...
      <div class="next-btn-wrap">
        <router-link class="btn btn--m btn--red" v-bind:to="{path:'orderConfirm',query:{'addressId':selectedId}}" >Next</router-link >
        <!-- 如果router-link跳转的是一个动态路由需要传递参数，那么to属性为动态属性应该设置为v-bind：to="" -->
      </div>
    </div>
  </div>
</div>
  <commonmodal v-bind:modalShow="modalDelAddress" v-on:close="closeModal">
        <p slot="message">确定要删除购物车中选中的商品吗？</p>
      <div slot="btnGroup">
        <a class="btn btn-m" href="javascript:void(0);" @click="closeModal">关闭</a>  
        <a class="btn btn-m" href="javascript:void(0);" @click="delAdSure(delAdress)">确认</a>  
      </div>
    </commonmodal>
...
import {setDefault} from "@/api/index.js";
  ...
  methods: {
    ...
    address(){
        getAddress().then(res=>{
            let addres =res.data
            if (addres.status == 0) {
          this.addressList = addres.result;
    ...
    setDefaultAd(addressId){
            setDefault(addressId).then(res=>{
            let Default =res.data
            if (Default.status == 0) {
                this.address();
        }
        }
    ...
    delAd(addressId){
        this.modalDelAddress = true
        this.delAdress=addressId
    },
    delAdSure(addressId){
...
<style>
.open{
   -webkit-transform: rotate-y(180deg);
